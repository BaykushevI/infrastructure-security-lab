...
| 80/443 | HTTP/HTTPS | admin UI attack surface; depends on auth/firmware | Medium |
...
- (less likely) **external attacker** unless router exposes WAN management/UPnP mapping

---

## Risk Assessment

Method: simple qualitative scoring (Impact x Likelihood).
...
### P0 (do first)
1. Disable **Telnet** (23/tcp) or restrict to isolated admin VLAN only.
2. Disable **UPnP** (1900/tcp / IGD behavior) unless explicitly needed.

### P1
3. Harden router admin access:
   - strong unique admin password
   - disable remote management from WAN
   - restrict admin UI to a single trusted device if possible
4. Update firmware; disable WPS; review firewall defaults.
